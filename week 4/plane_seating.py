...
def purchase_economy_plus(plane,economy_sold,name):
    ...
    ### TRY TO GET 1ST ROW SEATS IF NO WINDOW SEATS
    front_cols_order = [x for x in range(cols)]
    random.shuffle(front_cols_order)    #shuffle the seats in the front row

    for col in front_cols_order:
        if plane[0][col] in ["win","avail"]:    #if window seat or available seat in front row is available
            plane[0][col] = name                #we assign the name there
            return  plane
...
def purchase_economy_block(plane,economy_sold,number,name):
    ...
    seats_avail = seats_avail - get_number_economy_sold(economy_sold)       #isnt this just the get_avail seats?
...
def fill_plane(plane):
    ...
    return plane                    #I'm confused -- seat_economy seats economy seats, not economy plus passengers
                                    #Why would we use seat_economy to be assigning seats to economy plus?
    
    
def main():
    plane = create_plane(10,5)
    print("Plane initial seating chart:")
    print(get_plane_string(plane))
    print("total seats:", get_total_seats(plane))


    economy_sold = {'Robinson':3, 'Lee':2 }
    print("regular economy seats sold: ",get_number_economy_sold(economy_sold))
    print("remaining seats: ",get_avail_seats(plane,economy_sold))


    plane = fill_plane(plane)
    print("Here's what the filled up plane looks like:")
    print(get_plane_string(plane))
